	...
	<div class="container">
		<%
			...
			if (search == null) {
				...
				list = DAO.getNoticeList("SELECT * FROM Notice WHERE notice_title Like \"%" + search
						+ "%\" ORDER BY notice_num desc LIMIT ?, 20", Integer.parseInt(pageNum));
				noticeCount = DAO.getNoticCount(
						"SELECT COUNT(*) as cnt FROM Notice WHERE Notice.notice_title Like \"%" + search + "%\"");
			}
		...
		<div class="alert-link text-center">
			<ul class="pagination justify-content-center">
				...
				<li
					...
					href='?page=<%=pageNums[i]%><%=((search == null) ? "" : "&search=" + search)%>'>
						<%=pageNums[i]%>
				</a>
				</li>
				<%
